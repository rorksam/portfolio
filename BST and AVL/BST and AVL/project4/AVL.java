...
public class AVL<E extends Comparable<E>> implements Iterable<E>, Cloneable {
	...
	public static void main(String[] args) {
		...
		System.out.println("Welcome to the AVL Testing program. What would"
		...
		while(!end) {
			...
			if(decision.equalsIgnoreCase("add")) {
				...
				AVL<Integer> equalsTester = new AVL<>();
	 ...
	 */
	public AVL(E[] collection) throws NullPointerException {
		try {
			if(collection == null) throw new NullPointerException();
		} catch (NullPointerException e) {
			System.err.println("AVL cannot be created with collection defined by null.");
		}
		size = 0;
	...
	/**
	 * This nested class represents a post-order iterator for an AVL. 
	 * 
	 * @author Sam Rork
	 * @version 12/11/2023
	 */
	private class PostOrder implements Iterator<E> {
		...
		/**
		 * Default constructor for a PostOrder iterator of an AVL.
		 */
		public PostOrder() {
			lastVisited = null;
			current = root;
	...
	/**
	 * This nested class represents a pre-order iterator for an AVL.
	 * 
	 * @author Sam Rork
	 * @version 12/11/2023
	 */
	private class PreOrder implements Iterator<E> {
		...
		/**
		 * Finds the final node in the PreOrder of an AVL. 
...
//			System.err.print("PreO next() visited.");
	...
	/**
	 * This nested class represents an in-order iterator for an AVL. 
	 * This also happens to be the default iterator for this AVL class.
	 * 
	 * @author Sam Rork
	 * @version 12/11/2023
	 */
	...
	/**
	 * This nested class represents an individual unit of an AVL, called a Node. 
	 * 
	 * @author Sam Rork
	 * 
	 * @version 12/11/2023
	...
	private static class Node<E> {
		...
		public int updateBF() throws NullPointerException{
			try {
				this.updateHeight();
				balanceFactor = this.right.height - this.left.height;
...
//		}
	...
	public Node<E> recAdd(Node<E> n, E e) {
		if(e.compareTo(n.data) < 0) {
			...
			if(n.right != null) {
				return recAdd(n.right, e);
			} else {
				n.right = new Node<E>(e, n);
				n.updateHeight();
				size ++;
				this.balance(n.right);
				return n.right;
			}
		} return null;
	}
	...
	public boolean equals(Object obj) { 
		if(obj.getClass() == this.getClass()) {
			if(((AVL<E>)obj).size() == this.size()) {
				for(E e : this) {
					if(!((AVL<E>)obj).contains(e)) {
						return false;
					}
				} return true;
			}
	 ...
	 *     to each node in the AVL to find where the desired node with data 
	...
	public boolean remove(Object o) throws ClassCastException, NullPointerException{
		try {
			...
			System.err.println("Cannot search for query 'null' in AVL.");
			return false;
		}
	}
	...
	/**
	 * This method iterates up a tree from a given Node n and checks each 
	 *     parent for an imbalance. Called when an item is added or removed
	 *     from the tree.
	 * @param n - Node to balance (and its parents)
	 * @return true if the tree changed as a result of this call.
	 */
	public boolean balance(Node<E> n) {
		if(n == null) return false;
		if(n.updateBF() >= 2 || n.balanceFactor <= -2) {
			findImbalance(n);
	...
	/**
	 * This method finds the type of imbalance found by balance() function 
	 *     and handles reassignment.
	 * @param n - Node which is not balanced
	 * @return true if the set changed as a result of this call
	 */
	public boolean findImbalance(Node<E> n) {
		int bf = n.balanceFactor;
		if(bf >= 2) {
			if(n.right != null && n.right.updateBF() == 1) { //no need to updateBF here, should be already updated from working up the parent balancing
				if(n == root) root = RRimbalance(n);
				else if(n.parent.left == n) n.parent.left = RRimbalance(n);
				else n.parent.right = RRimbalance(n);
				return true;
			} else if (n.right != null && n.right.updateBF() == -1) {
				if(n == root) root = RLimbalance(n);
				else if(n.parent.left == n) n.parent.left = RLimbalance(n);
				else n.parent.right = RLimbalance(n);
				return true;
			} else return false;
		} else if (bf <= -2) {
			if(n.left != null) {
				if(n.left.updateBF() >= 1) {
					if(n == root) root = LRimbalance(n);
					else if(n.parent.left == n) n.parent.left = LRimbalance(n);
					else n.parent.right = LRimbalance(n);
					return true;
				} else if (n.left.balanceFactor <= -1) {
					if(n == root) root = LLimbalance(n);
					else if(n.parent.left == n) n.parent.left = LLimbalance(n);
					else n.parent.right = LLimbalance(n);
					return true;
				} else return false;
			}
		}
	...
	/**
	 * Left-Left rotation method for AVL trees.
	 * @param A - Node where imbalance is 
	 * @return new root node of what used to be subtree A
	 * 
	 * Some of the following logic is taken from Professor 
	 *     John Ortega's Week 12 Slides in Hofstra University 
	 *     CSC017. However, that does not include parent reassignment.
	 */
	public Node<E> LLimbalance(Node<E> A) {		
		Node<E> B = A.left;
	...
	/**
	 * Right-Right rotation method for AVL trees.
	 * @param A - Node where imbalance is 
	 * @return new root node of what used to be subtree A
	 * 
	 * Some of the following logic is taken from Professor 
	 ...
	 *     CSC017. However, that does not include parent reassignment.
	 */
	public Node<E> RRimbalance(Node<E> A) { 
		Node<E> B = A.right;
	...
	/**
	 * Left-Right rotation method for AVL trees.
	 * @param A - Node where imbalance is 
	 * @return new root node of what used to be subtree A
	 * 
	 * Some of the following logic is taken from Professor 
	 ...
	 *     CSC017. However, that does not include parent reassignment.
	 */
	public Node<E> LRimbalance(Node<E> A) {
		Node<E> B = A.left;
		Node<E> C = B.right;
	...
	/**
	 * Right-Left rotation method for AVL trees.
	 * @param A - Node where imbalance is 
	 * @return new root node of what used to be subtree A
	 * 
	 * Some of the following logic is taken from Professor 
	 ...
	 *     CSC017. However, that does not include parent reassignment.
	 */
	public Node<E> RLimbalance(Node<E> A) {
		Node<E> B = A.right;
		Node<E> C = B.left;
